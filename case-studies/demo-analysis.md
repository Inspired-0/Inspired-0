...
Новые встречи проходят фильтр по дате, названию и исключающим словам. После этого они регистрируются в SQLite со статусом `new`.
...
Проверенный результат сначала записывается в SQLite со статусом `in_progress`. Затем система обновляет строку встречи в Google Sheets и отдельно сохраняет обоснования оценок.
...
sequenceDiagram
    ...
    participant DB as SQLite только для чтения
...
Произвольные запросы к SQLite защищены на нескольких уровнях. Допускается один запрос `SELECT` или `WITH SELECT`. Встроенный механизм SQLite запрещает запись, изменение схемы, подключение других баз и служебные команды. Время выполнения, число строк, размер ответа и длина отдельных значений ограничены.
...
SQLite хранит состояние каждой встречи и результат оценки. После остановки система продолжает с последнего подтверждённого этапа. Записанный результат не оценивается повторно только из-за ошибки Google Sheets.
...
Проверки покрывают загрузку из двух источников, дубли, ручной отбор, расчёт доли речи, роли участников, валидацию ответа модели, поиск цитат и продолжение работы после сбоя. Отдельные наборы тестов проверяют агента, маршрутизацию сообщений, права доступа, полнотекстовый поиск, смысловой поиск и защиту запросов к SQLite.
...
- Качество результата зависит от записи и расшифровки встречи.
- Найденная цитата подтверждает конкретный фрагмент, но не весь вывод модели.
- Фактический пересказ модели хранится отдельно, но не подтверждается целиком одной цитатой.
- Смысловой поиск рассчитан на текущий объём в несколько сотен встреч. При росте примерно до тысячи встреч его разумно заменить векторным поиском.
- SQLite подходит для одного сервера. Для нескольких серверов понадобится отдельная база данных.
- Спорные оценки сохраняют статус `needs_review` и требуют решения человека.
